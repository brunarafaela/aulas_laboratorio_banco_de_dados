...
ALTER TABLE turma ADD (cert_instr CHAR(6) NOT NULL);
...
ALTER TABLE turma ADD CONSTRAINT chk_situ_turma CHECK (SITUACAO_TURMA IN ('Ativa', 'Cancelada', 'Encerrada'));
ALTER TABLE matricula ADD CONSTRAINT chk_situ_matricula CHECK (SITUACAO_MATR IN ('Cursando', 'Cancelada', 'Aprovado', 'Reprovado'));


-- c) Renomeie alguma coluna;
ALTER TABLE aula RENAME COLUMN MATERIAL TO material_aula;
...
ALTER TABLE aluno MODIFY login VARCHAR(12);
...
ALTER TABLE matricula MODIFY APROVEITAMENTO_PROVA_CERT DEFAULT 0.00;
ALTER TABLE matricula MODIFY APROVEITAMENTO_FINAL DEFAULT 0.00;
ALTER TABLE participacao_aula MODIFY aproveitamento_atividade DEFAULT 0.00;
...
INSERT INTO USO_SOFTWARES_CURSO VALUES (
	'CCNA1',
	'SQL'
);
...
-- Preencha o nome com o conteúdo que existe na coluna original em Software; estabeleça o relacionamento incluindo
--  o id do fabricante na tabela software e atualizando os dados. Posteriormente exclua a coluna fabricante.

DROP TABLE fabricante_sftw CASCADE CONSTRAINTS;
CREATE TABLE fabricante_sftw
(
    id_fabricante SMALLINT NOT NULL,
    FABRICANTE VARCHAR2(60) NOT NULL,
    pais_fabricante VARCHAR2(20) NOT NULL
);
ALTER TABLE fabricante_sftw ADD CONSTRAINT pk_fabr_sftw
PRIMARY KEY (id_fabricante);
INSERT INTO fabricante_sftw VALUES (1, 'Cisco Systems Inc', 'EUA');
INSERT INTO fabricante_sftw VALUES (2, 'Oracle Corporation', 'EUA');
ALTER TABLE software ADD id_fabricante SMALLINT;
UPDATE software s
 SET s.id_fabricante = (SELECT fs.id_fabricante 
 	 					FROM fabricante_sftw fs
 	 					WHERE UPPER(s.id_fabricante) LIKE '%'||UPPER(fs.FABRICANTE)||'%');
 ALTER TABLE software MODIFY id_fabricante NOT NULL;
 ALTER TABLE software ADD CONSTRAINT fk_fabr_softw FOREIGN KEY (id_fabricante)
 REFERENCES fabricante_sftw (id_fabricante);
